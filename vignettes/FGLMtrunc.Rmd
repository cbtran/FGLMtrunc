...
We fit the model using 50 B-spline basis with default `degree=3` for cubic splines. Since argument `grid` is not specified, an equally spaced sequence of length $p=101$ on $[0,1]$ interval (including boundaries) will automatically be used. `fglm_trunc` also supports parallel computing to speed up the running time of tuning regularization parameters. We will use `num.cores=2` throughout this vignette.
```{r}
fit = fglm_trunc(Y_linear, Xcurves_linear, nbasis = 50, num.cores = 2)
```
...
fit2 = fglm_trunc(Y_linear, Xcurves_linear, grid = timeGrid, knots = knots_n, num.cores = 2)
```
...
```{r}
fit4 = fglm_trunc(Y_logistic, Xcurves_logistic, family="binomial", nbasis = 50, num.cores = 2)
```
...
```{r}
fit_scalar = fglm_trunc(X.curves=Xcurves_linear, Y=Y_scalar, S=S, nbasis = 50, num.cores = 2)
fit_scalar
```
